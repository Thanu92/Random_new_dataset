...
files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/RANDOM", pattern="^[0-9]{1,2}_[0-9]{1,2}_new", full.names=TRUE, recursive=FALSE)
...
bp <- ggboxplot(RF_metric,x="sample_completeness",y="RF_distance", color = "sample_completeness",palette = c("#6BAED6" ,"#4292C6", "#2171B5", "#084594","#003366")
                ,main="Boxplot of RF distance for random samples based on sample completeness",add= "point", width=0.8,outlier.shape = NA)
...
#Stratified RF distance
#RF_dist_sample_types.csv or RF_dist_sample_type.csv
...
ezANOVA(data = Random_Correct_position, dv = correct_placement_percentage, wid = id, within = sample_completeness)
...
#QQ plot draws the correlation between a given data and the normal distribution. Create QQ plots for each level backbone tree(sample completeness)
ggqqplot(Random_Correct_position, "correct_placement_percentage", facet.by = "sample_completeness",color = "sample_completeness")
...
pwc <- Random_Correct_position %>%
  pairwise_t_test(
    correct_placement_percentage ~ sample_completeness, paired = TRUE,
    p.adjust.method = "bonferroni"
  )
pwc
...
#Calculate the repeated measures correlation coefficient. id is variable giving the subject name/id for each observation. RF_distance is a numeric variable giving the observations for one measure, sample_completeness is a numeric variable giving the observations for the second measure, longdat is the data frame containing the variables.
rmcorr(id, correct_placement_percentage , sample_completeness, Random_Correct_position_new )

#non-parametric for repeated messures anova is friedman test

res.fried <- Random_Correct_position %>% friedman_test(correct_placement_percentage ~ sample_completeness |id)
res.fried
#The correct species percentage was statistically significantly different at the different time points during the diet, X2(2) = 30, p = 0.00000138
...
#Kendall’s W uses the Cohen’s interpretation guidelines of 0.1 - < 0.3 (small effect), 0.3 - < 0.5 (moderate effect) and >= 0.5 (large effect). Confidence intervals are calculated by bootstap.
Random_Correct_position %>% friedman_effsize(correct_placement_percentage ~ sample_completeness |id)
#A large size is detected W=1
...
# Pairwise comparisons using paired Wilcoxon signed-rank test. P-values are adjusted using the Bonferroni multiple testing correction method.
pwc <- Random_Correct_position%>%
  wilcox_test(correct_placement_percentage ~ sample_completeness, paired = TRUE, p.adjust.method = "bonferroni")
pwc
#In this csv file Random,stratified and biased results are included. As I need Random results first I subset another dataframe from Correct_position dataset
Stratified_Correct_position <- Correct_position[51:100,]
...
class(Random_Correct_position)
ezANOVA(data = Stratified_Correct_position, dv = correct_placement_percentage, wid = id, within = sample_completeness)
...
#visualization
#Create a violin plot
violin_plot <- ggplot(Stratified_Correct_position, aes(x=sample_completeness, y=correct_placement_percentage,color=sample_completeness))+ geom_violin(trim = FALSE)+theme(legend.position="none")+geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5,binwidth = 0.01)+labs(title="The box plot of Correct position percentage for sampling completeness of stratified sampling")
violin_plot
...
#QQ plot draws the correlation between a given data and the normal distribution. Create QQ plots for each level backbone tree(sample completeness)
ggqqplot(Stratified_Correct_position, "correct_placement_percentage", facet.by = "sample_completeness",color = "sample_completeness")
