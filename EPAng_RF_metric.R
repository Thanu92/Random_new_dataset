...
library(phangorn)
library(ggplot2)
files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/RANDOM/EPAng", pattern="*_[0-9]{1,2}.nwk", full.names=TRUE, recursive=FALSE)
...
names(treeList) <- gsub("/home/thanu/Desktop/FishData/New_dataset/RANDOM/EPAng/", "", files)
...
# check <- read.tree("RAxML20_1.txt")
# now <- read.tree("R80_8.nwk")
# now$tip.label
# #check$tip.label
# RF_R20_1 <- RF.dist(parseTreeComp,now,normalize = TRUE)
# R20_1$tip.label
# parseTreeComp$tip.label
...
order <- c(2, 12, 22, 32,# 20_10, 40_10, 60_10, 80_10
           ...
           9, 19, 29, 39,
           10, 20, 30, 40,
           1, 11, 21, 31) # 20_1, 40_1, 60_1, 80_1
...
colnames(RF_metric) <- c("20","40","60","80")
RF_metric <- stack(RF_metric)
head(RF_metric)
colnames(RF_metric) <- c("RF_distance","sample_completeness")
library(ggpubr)
bp <- ggboxplot(RF_metric,x="sample_completeness",y="RF_distance", color = "sample_completeness",palette = c("#6BAED6" ,"#4292C6", "#2171B5", "#084594")
