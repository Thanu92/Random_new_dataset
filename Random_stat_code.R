...
files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/RANDOM", pattern="*_[0-9]{1,2}_new", full.names=TRUE, recursive=FALSE)
...
names(treeList) <- gsub("/home/thanu/Desktop/FishData/New_dataset/RANDOM/", "", files)
...
#Correct species position
Correct_position<- read.csv("Correct_Species_Placement.csv")

#In this csv file Random,stratified and biased results are included. As I need Random results first I subset another dataframe from Correct_position dataset
Random_Correct_position <- Correct_position[1:40,]
...
Random_Correct_position %>%
  group_by(sample_completeness) %>%  
  shapiro_test(correct_position_percentage)
...
#QQ plot draws the correlation between a given data and the normal distribution. Create QQ plots for each level backbone tree(sample completeness)
ggqqplot(Random_Correct_position, "correct_position_percentage", facet.by = "sample_completeness",color = "sample_completeness")
...
pwc <- Random_Correct_position %>%
  pairwise_t_test(
    correct_position_percentage ~ sample_completeness, paired = TRUE,
    p.adjust.method = "bonferroni"
  )
pwc
...
#Calculate the repeated measures correlation coefficient. id is variable giving the subject name/id for each observation. RF_distance is a numeric variable giving the observations for one measure, sample_completeness is a numeric variable giving the observations for the second measure, longdat is the data frame containing the variables.
rmcorr(id, correct_position_percentage , sample_completeness, Random_Correct_position_new )

#non-parametric for repeated messures anova is friedman test

res.fried <- Random_Correct_position %>% friedman_test(correct_position_percentage ~ sample_completeness |id)
res.fried
...
Random_Correct_position %>% friedman_effsize(correct_position_percentage ~ sample_completeness |id)
...
pwc <- Random_Correct_position%>%
  wilcox_test(correct_position_percentage ~ sample_completeness, paired = TRUE, p.adjust.method = "bonferroni")
pwc
#In this csv file Random,stratified and biased results are included. As I need Random results first I subset another dataframe from Correct_position dataset
Stratified_Correct_position <- Correct_position[41:80,]
...
class(Random_Correct_position)
ezANOVA(data = Stratified_Correct_position, dv = correct_position_percentage, wid = id, within = sample_completeness)
...
#visualization
#Create a violin plot
violin_plot <- ggplot(Stratified_Correct_position, aes(x=sample_completeness, y=correct_position_percentage,color=sample_completeness))+ geom_violin(trim = FALSE)+theme(legend.position="none")+geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5,binwidth = 0.01)+labs(title="The box plot of Correct position percentage for sampling completeness of stratified sampling")
violin_plot
...
#QQ plot draws the correlation between a given data and the normal distribution. Create QQ plots for each level backbone tree(sample completeness)
ggqqplot(Stratified_Correct_position, "correct_position_percentage", facet.by = "sample_completeness",color = "sample_completeness")
...
#Calculate the repeated measures correlation coefficient. id is variable giving the subject name/id for each observation. RF_distance is a numeric variable giving the observations for one measure, sample_completeness is a numeric variable giving the observations for the second measure, longdat is the data frame containing the variables.
rmcorr(id, correct_position_percentage , sample_completeness, Stratified_Correct_position_new )
