...
#After adding 99% sampling completeness 
#did changes to RF distance script
...
#For EPAng RAndom sample,

files <- list.files(path="/home/thanu/Desktop/FishData/New_dataset/RANDOM", pattern="EPA[0-9]{1,2}_[0-9]{1,2}R_new.nwk", full.names=TRUE, recursive=FALSE)
treeList <- foreach(i=1:length(files)) %do% read.tree(files[i])
...
RF_Random_Plot <- ggplot(RF_metric , aes(GRF)) +
...
RF_Random_Plot + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
# Plot Robinson-Fould distances 
Path_Random_Plot <- ggplot(P_metric , aes(GRF)) +
...
#remove background (remove backgroud colour and border lines, but does not remove grid lines) using theme(panel.background = element_blank())
#add axis line using heme(axis.line = element_line(colour = "black"))
Path_Random_Plot + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                      panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
getwd()
